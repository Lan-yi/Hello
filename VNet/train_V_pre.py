...
# @atexit.register
# def goodbye():
#     draw_loss(Loss_list, sets.v_epoch+1)
#     print("Bye")
...
pretrained_model_dict = torch.load(checkpoint_path)
...
# checkpoint = torch.load(r'.\model\vnet_breast_0925.pth', map_location=device)
# model.load_state_dict(checkpoint)   #模型参数
# model = torch.nn.Sequential( *( list(model.modules())[:-2] ) )
# model.out_conv = nn.Conv3d(16, 4, 1, padding=0)
...
#setting
sets = parse_opts()
img_list = './dataset/NACData_VNet_hundreds/train_3.txt' #训练文本
batch_size = 8
num_workers = 0
pin_memory = True
epochs = 1000
save_intervals = 10
save_folder = "./trails/models_3/{}".format('VNet')    #模型参数保存路径
best_acc = 0.0
#save_path = './Vet.pth'
...
image_path = os.path.join(data_root, 'N5N6')    #
