...
class ResidualConvBlock(nn.Module):
    def __init__(self, n_stages, n_filters_in, n_filters_out, normalization=None, activation=None, expand_chan=False,
                 drop_out_need=True):
...
class VNet(nn.Module):
    def __init__(self, n_channels, n_classes, n_filters=16, normalization=None, activation=None, depth=3):
        ...
        self.depth = depth
        if n_channels > 1:
            self.block1 = ResidualConvBlock(1, n_channels, n_filters, normalization=normalization,
                                            activation=activation, expand_chan=True)
        else:
            self.block1 = ResidualConvBlock(1, n_channels, n_filters, normalization=normalization,
                                            activation=activation)

        self.block1_dw = DownsamplingConvBlock(n_filters, 2 * n_filters, normalization=normalization,
                                               activation=activation)

        self.block2 = ResidualConvBlock(2, n_filters * 2, n_filters * 2, normalization=normalization,
                                        activation=activation)
        self.block2_dw = DownsamplingConvBlock(n_filters * 2, n_filters * 4, normalization=normalization,
                                               activation=activation)

        self.block3 = ResidualConvBlock(3, n_filters * 4, n_filters * 4, normalization=normalization,
                                        activation=activation)
        self.block3_dw = DownsamplingConvBlock(n_filters * 4, n_filters * 8, normalization=normalization,
                                               activation=activation)
        self.block3_up = UpsamplingDeconvBlock(n_filters * 4, n_filters * 2, normalization=normalization,
                                               activation=activation)

        self.block4 = ResidualConvBlock(3, n_filters * 8, n_filters * 8, normalization=normalization,
                                        activation=activation, drop_out_need=False)
        self.block4_up = UpsamplingDeconvBlock(n_filters * 8, n_filters * 4, normalization=normalization,
                                               activation=activation)

        self.block5 = ResidualConvBlock(3, n_filters * 4, n_filters * 4, normalization=normalization,
                                        activation=activation)
        self.block5_up = UpsamplingDeconvBlock(n_filters * 4, n_filters * 2, normalization=normalization,
                                               activation=activation)

        self.block6 = ResidualConvBlock(2, n_filters * 2, n_filters * 2, normalization=normalization,
                                        activation=activation)
        self.block6_up = UpsamplingDeconvBlock(n_filters * 2, n_filters, normalization=normalization,
                                               activation=activation)

        self.block7 = ResidualConvBlock(1, n_filters, n_filters, normalization=normalization, activation=activation)
        self.Out_conv = nn.Conv3d(n_filters, n_classes, 1, padding=0)  ###预训练的参数名 self.out_conv
