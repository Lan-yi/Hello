...
class Vnetdata23(Dataset):

    def __init__(self, root_dir, img_list, sets):
        #super(vnetdata22,self).__init__()
    ...
    def __getitem__(self, idx):
        ...
        if self.phase == "train":
            ...
            # data processing
            img_array, mask_array, volume_orig_affine, affine = self.__training_data_process__(img, mask)
            ...
            return img_array, mask_array, volume_orig_affine, affine
...
###################################################################3
    ...
    def __training_data_process__(self, data, label):
        ...
        volume_orig_affine = data.affine
        ...
        mask = mask_nifty_re.get_fdata()

        # drop out the invalid range
        volume, mask = self.__drop_invalid_range__(volume, mask)

        # crop data
        #data, label = self.__crop_data__(data, label)

        # resize data
        volume = self.__resize_data__(volume)
        mask = self.__resize_data__(mask)

        # normalization datas
        volume = self.__itensity_normalize_one_volume__(volume)

        return volume, mask, volume_orig_affine, affine
