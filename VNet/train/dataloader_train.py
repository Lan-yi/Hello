...
class dataset_1121(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == 'train':
            ...
            img_array, mask_array, orig_affine, affine = load_dataset_tr(img_name,label_name)
...
def load_dataset_tr(img_path, labelpath, normalize=True):

    volume, label, orig_affine, affine = load_data_tr(img_path, labelpath, normalize)
...
#         label_paths = self.path['label']
#         for i in range(len(img_paths)):
#             img, affine, affine_org, shape_org = load_dataset_ts(img_paths[i], self.mode)
#             label, affine, affine_org, shape_org = load_dataset_ts(label_paths[i], self.mode)
#             self.list.append(tuple((img, label)))
#
#
#         # img_paths = self.path['img']
...
#
#
# def load_dataset_ts(file_path, mode, normalize=True):
#     print(f"Loading and preprocessing {mode} data...")
#
#     volume, affine, affine_orig, shape = load_data_ts(file_path, normalize)
#
#
