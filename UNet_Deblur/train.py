...
#训练图像；训练标签；训练结果目录
# ---------------------------------#
dir_img = base / '8/train/images'
dir_mask = base / '8/train/masks/'
dir_checkpoint = 'checkpoints/checkpoints_8/'
    ...
    for epoch in range(epochs):
        ...
        with tqdm(total=n_train, desc=f'Epoch {epoch + 1}/{epochs}', unit='img') as pbar:
            for batch in train_loader:
                ...
                #true_masks = Variable(true_masks, requires_grad=True)   #######

                # if hasattr(torch.cuda, 'empty_cache'):    ##########添加释放缓存
                ...
                if global_step % max((n_train // (10 * batch_size)), 1)== 0:
                    # 验证集评估模型
                    #with torch.no_grad():    #########不算梯度
                    val_score = eval_net(net, val_loader, device)       #并没有计算梯度，无需上述语句
